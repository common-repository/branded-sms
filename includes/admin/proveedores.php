...
function apg_sms_envia_sms( $apg_sms_settings, $telefono, $mensaje ) {
	switch ( $apg_sms_settings['servicio'] ) {
		case "hostandsoft":
			if($apg_sms_settings[ 'servidor_hostandsoft' ] == 'bsms.hostandsoft.com'){
				$argumentos[ 'body' ]		= [ 
					'action' 				=> 'send-sms',
					'api_key' 				=> $apg_sms_settings['contrasena_hostandsoft'],
					'sms' 					=>  $mensaje,
					'to' 					=> $telefono,
					'from'					=> $apg_sms_settings['identificador_hostandsoft'],
				];
				$respuesta					= wp_remote_post( "https://" . $apg_sms_settings[ 'servidor_hostandsoft' ] . "/app/sms/api", $argumentos );
			}else{
				$respuesta = wp_remote_get( "http://sms.hostandsoft.com:80/api/?sender=" . $apg_sms_settings['identificador_hostandsoft'] . "&username=" . $apg_sms_settings['usuario_hostandsoft'] . "&password=" . $apg_sms_settings['contrasena_hostandsoft'] . "&receiver=" . $telefono . "&msgdata=" . apg_sms_codifica_el_mensaje( $mensaje ) );
			}
		break;
		case "brandedsmsio":
			$argumentos[ 'body' ]		= [ 
				'action' 				=> 'send-sms',
				'api_key' 				=> $apg_sms_settings['contrasena_brandedsmsio'],
				'sms' 					=> $mensaje,
				'to' 					=> $telefono,
				'from'					=> $apg_sms_settings['identificador_brandedsmsio'],
			];
			$respuesta					= wp_remote_post( "https://brandedsms.io/app/sms/api", $argumentos );
		break;
		case "adlinks":
			$url						= add_query_arg( [
	...
	if ( isset( $apg_sms_settings['debug'] ) && $apg_sms_settings['debug'] == "1" && isset( $apg_sms_settings['campo_debug'] ) ) {
		$correo	= __( 'Mobile number:', 'branded-sms' ) . "\r\n" . $telefono . "\r\n\r\n";
		$correo	.= __( 'Message: ', 'branded-sms' ) . "\r\n" . $mensaje . "\r\n\r\n"; 
		$correo	.= __( 'Gateway answer: ', 'branded-sms' ) . "\r\n" . print_r( $respuesta, true );
		wp_mail( $apg_sms_settings['campo_debug'], 'Branded SMS', $correo, 'charset=UTF-8' . "\r\n" ); 
	}
}